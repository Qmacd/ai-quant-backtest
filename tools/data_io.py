...
class DataIO():
    ...
    def input_futureInformation():
        ...
        for name in name_dict:
            ...
            names.append(name_dict[name]['name'])
    ...
    @staticmethod 
    def input_futureInformation_NoWenhua():
        ...
        #for name in names:'''
        for name in name_dict:
            if name_dict[name]['wh_code']!="7186":
            # if not math.isnan(name_dict[name]['margin']):
            # #if 1:
                wh_codes.append(name_dict[name]['wh_code'])
                margins.append(name_dict[name]['margin'])
    ...
    @staticmethod
    def input_bitcoinInformation():
        """
        交互式地获取用户想要回测的品种名称、回测起始日期和结束日期。
        :return: 用户选择的品种代码wh_code列表、回测起始日期和结束日期
        """
        # 显示股票信息字典
        name_dict = show_bitcoin_codes_from_mysql()
        wh_codes = list()  # 存储用户选择的股票文华代码
        names = list()  # 存储用户输入的股票名称
        margins = list()  # 存储用户选择的期货保证金比例
        mults = list()  # 存储用户选择的期货合约乘数
        ...
        # print(name_dict)
        # 选回测周期
        period_options = {"1": ('day', 'day'), "2": ('hour', 'hour'), "3": ('15min', 'min'), "4": ('5min', 'min'),
                          "5": ('1min', 'min'), "6": ('5s', 's')}
        period_input = input("请选择回测周期：\n1.一天\n2.1小时\n3.15分钟\n4.5分钟\n5.1分钟\n6.5秒钟\n").strip()
        if period_input not in period_options:
            print("无效选项，请重新运行程序并选择有效选项。")
        ...
        while True:
            start_date = DataGet.get_str_to_datetime_btc(
                "请按说明格式输入回测起始日期或时间：\n（例如：若为2024年12月16日15点30分15秒则应输入：20241216153015）\n",
                has_time)

            end_date = DataGet.get_str_to_datetime_btc(
                "请按说明格式输入回测结束日期或时间：\n（例如：若为2024年12月16日15点30分15秒则应输入：20241216153015）\n",
                has_time)
            if end_date <= start_date:
                print("结束日期或时间不可早于或等于起始日期或时间！！！请重试！！！")
                continue
        ...
        DataIO.end_date = end_date
        print("[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]")
        print(wh_codes,names, start_date, end_date, period, margins, mults,CN_names)
        return wh_codes, names, start_date, end_date, period, margins, mults, CN_names  # 返回股票代码列表和日期
...
def show_future_codes_from_mysql():
    ...
    langzi = input('选择合约篮子: 1.商品期货 2.有色金属\n')
    if langzi == '1':
        ...
        query = "SELECT 期货名, 保证金比例, 合约乘数, 别名, wh_code FROM color_codes"
        data = pd.read_sql(query, con=connection)
...
def show_bitcoin_codes_from_mysql():
    """
    从MySQL数据库中读取所有上市交易的期货代码，并返回期货名称、保证金比例、合约乘数以及文化码的对应字典。
    :return: 包含期货名称、保证金比例、合约乘数和文化码的字典
    """
    ...
    print("虚拟货币回测")
    while True:
        # config.lanzi = input('选择合约篮子: \n1.虚拟货币篮子 \n2.测试篮子(目前为空不要选择！)\n')
        config.lanzi = '1'
        if config.lanzi in ['1','2']:
            break
        else:
            print("无效篮子，请重新输入")
    lanzi = config.lanzi
