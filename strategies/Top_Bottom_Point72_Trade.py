...
class Top_Bottom_Point72_Trade(bt.Strategy):
    params=(
    ...
    def __init__(self):
        ...
        self.max_drawdown=0#最大回撤的值
        self.max_drawdown_percent=0#最大回撤相对于回撤开始点的百分比
        self.max_total_value = self.init_cash  # 初始化为初始资金
        self.min_total_value = self.init_cash  # 初始化为初始资金
        self.nums_of_open=10
        #总共交易8个品种
            ...
            if self.cal_next_bar_is_last_trading_day(test_data):
                ...
                for data in self.datas:
                    ...
                    if data.cnname=="文华商品":
                        ...
                        Log.log(f"文华商品分数:{wenhua_score}",dt=current_date)
                ...
                if wenhua_score >= 72:
                    # 强多头行情：前7做多，后3做空
                    buy_wait = self.report.head(7)['data名'].tolist()
                    sell_wait = self.report.tail(3)['data名'].tolist()
                elif wenhua_score <= -72:
                    # 强空头行情：前3做多，后7做空
                    buy_wait = self.report.head(3)['data名'].tolist()
                    sell_wait = self.report.tail(7)['data名'].tolist()
                else:
                    # 震荡行情：前5做多，后5做空
                    buy_wait = self.report.head(5)['data名'].tolist()
                    sell_wait = self.report.tail(5)['data名'].tolist()
                ...
                for data in buy_wait:
                    if self.getposition(data).size==0:
                        ...
                        lots=(fund*0.08)//(data.close[0]*margin_percent*mult)
